...
This demo demonstrates power management capabilities on UniRTOS, including boot cause detection, device reset, shutdown control, power key callback, and battery status polling.

- Demonstrates querying boot cause (PSM wake, power key, reset, watchdog, panic, software reset)
- Demonstrates normal device reset with `qosa_power_reset`
- Demonstrates normal device power-down with `qosa_power_down`
- Demonstrates power key callback registration with `qosa_pwrkey_callback_register` for event-driven shutdown
- Demonstrates periodic battery status polling: charging status, charging level, and voltage
